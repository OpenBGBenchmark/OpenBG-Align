...
from capture.module_cross import CrossModel
from capture.until_config import PretrainedConfig
...
class BertEncoder(nn.Module):
    ...
    def info_nce_loss(self, features, batch_size,n_views):
        ...
        # print("labels2: ", labels.size())
        # print("similarity_matrix2: ", similarity_matrix.size())
    ...
    def forward(
        ...
        # return_t_feature = pooled_feature_t.clone()
        # return_v_feature = pooled_feature_v.clone()
        features = torch.cat([pooled_feature_v, pooled_feature_t], dim=0)
...
class BertPreTrainingHeads(nn.Module):
    def __init__(self, config, bert_model_embedding_weights):
        ...
        self.v_weight_fc=nn.Linear(config.bi_hidden_size,1)
        self.t_weight_fc=nn.Linear(config.bi_hidden_size,1)
        self.vil_weight_fc=nn.Linear(config.bi_hidden_size*2,config.bi_hidden_size)
    ...
    def forward(
        ...
        if self.fusion_method == 'sum':
            ...
            pooled_output=self.dropout(self.v_weight_fc(pooled_output_v)
                                       +self.t_weight_fc(pooled_output_t))
        elif self.fusion_method=="concat":
            concat_feat=torch.cat((pooled_output_t,pooled_output_v),dim=-1)
            pooled_output=self.dropout(self.vil_weight_fc(concat_feat))
        else:
            assert False
...
class BertModel(BertPreTrainedModel):
    def __init__(self, config):
        ...
        # self.cross_config=PretrainedConfig.from_json_file("/data1/xl/multi_modal/CAPTURE/model/cross-base/cross_config.json")
        cross_config_temp={
            "attention_probs_dropout_prob": 0.1,
            "hidden_act": "gelu",
            "hidden_dropout_prob": 0.1,
        ...
        self.cross_config=PretrainedConfig.from_dict(cross_config_temp)
    ...
    def _get_cross_output(self, sequence_output, visual_output,
        ...
        # print("concat_features", concat_features.size())
        # print("concat_type", concat_type.size())
        # print("concat_mask", concat_mask.size())
    ...
    def forward(
        ...
        encoded_layers_t, encoded_layers_v,CLR_loss, all_attention_mask = self.encoder(
            embedding_output,
            v_embedding_output,
        ...
        # print("encoded_layers_t: ",encoded_layers_t[-1].size())

        sequence_output_t = encoded_layers_t[-1]
        sequence_output_v = encoded_layers_v[-1]

        # co encoder
        # print("sequence_output_t: ",sequence_output_t.size())
        # print("sequence_output_v: ",sequence_output_v.size())
        # print("attention_mask: ",attention_mask.size())
        # print("image_attention_mask: ",image_attention_mask.size())
        ...
        return encoded_layers_t, encoded_layers_v, pooled_output_t, pooled_output_v,CLR_loss, all_attention_mask
...
class CaptureForMultiModalPreTraining_SimCLR(BertPreTrainedModel):
    """BERT model with multi modal pre-training heads.
    """

    def __init__(self, config):
        super(CaptureForMultiModalPreTraining_SimCLR, self).__init__(config)

        self.bert = BertModel(config)
        self.cls = BertPreTrainingHeads_woITM(
            config, self.bert.embeddings.word_embeddings.weight
        )
        self.fc=nn.Linear(config.bi_hidden_size,config.hidden_size)
    ...
    def _mean_pooling_for_similarity(self, sequence_output, visual_output, attention_mask, video_mask):
        ...
        text_image_out=torch.mean(torch.stack((text_out,video_out)),dim=0)

        return text_out, video_out, text_image_out

    def forward(
        self,
        input_ids, # B X seq
        image_feat, # B X box X 2048
        image_loc, # B X box X 5
        token_type_ids=None,
        attention_mask=None,
        image_attention_mask=None, # B x box : while less than max_box_num, padding
        masked_lm_labels=None, # B x seq
        image_label=None, # 1 x 36 -- B X box : if is mask
        image_target = None, # 1 x 36 x 1601 -- B x box x 1601 : the prediction scores for each box
        next_sentence_label=None, # B : match or not
        output_all_attention_masks=False,
        ...
        sequence_output_t, sequence_output_v, \
        pooled_output_t, pooled_output_v, CLR_loss, all_attention_mask = self.bert(
            input_ids, # 1 x 30
            image_feat, # 500 x 100 x 2048
            image_loc, # 500 x 100 x 5
            token_type_ids, # 1 x 30
            attention_mask, # 1 x 30
            image_attention_mask, # 500 x 100
            output_all_encoded_layers=False,
            output_all_attention_masks=output_all_attention_masks
        )
        ...
        if masked_lm_labels is not None and next_sentence_label is not None and image_target is not None:
            ...
            if return_features == False:
                return prediction_scores_t, prediction_scores_v, all_attention_mask
            else:
                return prediction_scores_t, prediction_scores_v\
                    , all_attention_mask, pooled_output_t, pooled_output_v


class Capture_ForClassification(BertPreTrainedModel):
    def __init__(self,config,num_classes):
        super(Capture_ForClassification,self).__init__(config)
        self.bert=BertModel(config)
        self.cls = BertPreTrainingHeads_woITM(
            config, self.bert.embeddings.word_embeddings.weight
        )
        ...
        self.dense2=nn.Linear(config.bi_hidden_size,num_classes)
        self.apply(self.init_bert_weights)
        self.loss_fct = CrossEntropyLoss(ignore_index=-1)
    ...
    def forward(
        ...
        sequence_output_t, sequence_output_v, \
        pooled_output_t, pooled_output_v,CLR_loss,\
        all_attention_mask = self.bert(
            input_ids, # 1 x 30
            image_feat, # 500 x 100 x 2048
            image_loc, # 500 x 100 x 5
            token_type_ids, # 1 x 30
        ...
        pooled_output=torch.cat((pooled_output_t,pooled_output_v),dim=-1)
...
class Capture_ForRetrieval(BertPreTrainedModel):
    def __init__(self,config):
        super(Capture_ForRetrieval,self).__init__(config)
        self.bert=BertModel(config)
        self.cls = BertPreTrainingHeads_woITM(
            config, self.bert.embeddings.word_embeddings.weight
        )
        self.dense1=nn.Linear(config.hidden_size+config.v_hidden_size,config.hidden_size)
        self.apply(self.init_bert_weights)
        self.loss_fct = CrossEntropyLoss(ignore_index=-1)
    ...
    def forward(
        ...
        pooled_output=torch.cat((pooled_output_t,pooled_output_v),dim=-1)
        hidden_state=self.dense1(pooled_output)
        return pooled_output_t,pooled_output_v,hidden_state
